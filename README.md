...
Phase scope: `0AX` - simulated voice confirmation protocol (builds on `0A + 0S + 0R virtual chat + 0B retrieval bridge + 0C BOH link + 0S/S4 bridge emulator + 0S/S3 provider harness + 0P personality + 0V voice + 0M manifest + 0X artifacts + 0Y parity + 0V/AUDIO9 animated analyzer + 0T/CHAT governed tools + 0U proposal review + 0W execution audit + 0Q read-only policy + 0L time lane + 0G git status lane + 0F filesystem read lane + 0J active read-only chat routing + 0K fetch/planning seeds + 0N audit replay hardening + 0D lifecycle visibility + 0E BOH proposal lane + 0I proposal filters + 0H permission metadata + 0AA contract manifest + 0AB policy snapshot + 0AC argument validation + 0AD gate evaluation + 0AE review scope + 0AF tool readiness + 0AG completion report + 0AH task planner + 0AI plan queue + 0AJ plan review + 0AK step proposals + 0AL execution requests + 0AM result binding + 0AN guided advance + 0AO chat planning + 0AP truthful tool context + 0V/AUDIO10 spectrum fidelity + 0AQ launch-root hardening + 0AR chat plan control + 0AS proposal/receipt awareness + 0AT next-action guidance + 0AU dashboard guided shortcuts + 0AV simulated voice-command routing + 0AW deterministic tool awareness`).

Status: Metis now has a simulated voice confirmation protocol for pending governed proposals.
`POST /metis/voice/confirm` accepts caller-supplied recognized text, emits redacted voice-confirmation
events, reads back the pending proposal when phrasing is ambiguous, and only applies review when the
spoken text contains an explicit proposal ID plus approve/deny intent. It never requests execution,
never grants standing approval, and remains blocked by mic cutoff. There is still no real microphone,
wake word, or STT runtime in this phase.

Phase 0AX implemented:

- Added `POST /metis/voice/confirm` for simulated voice approval/denial of one pending proposal.
- Added `metis_voice_confirmation.v0.1` redacted STT-style events and
  `metis_voice_confirmation_readback.v0.1` readback metadata.
- Ambiguous phrases such as "yes" return `readback_required` and leave the proposal pending.
- Explicit phrases such as `confirm approve proposal_...` or `deny proposal_...` reuse the existing
  single-proposal review path and still set `execution_allowed=false`.
- `cancel proposal_...` leaves the proposal pending.
- Mic cutoff blocks voice confirmation before review changes.
- Added tests proving explicit confirmation, denial, cancellation, readback, mic cutoff blocking,
  and no execution/audit receipt creation.
- Verification after Phase 0AX: `267 passed` under Python 3.11.

Previous Phase 0AW status: Metis answers tool/capability questions through a deterministic Brain-owned route,
...
- Added `POST /metis/voice/command` for simulated recognized voice commands.
- Voice commands route through `/metis/chat` so tool awareness, proposal gates, next-action guidance,
  and LLM tool context stay canonical.
- Added redacted `provider_event` records for simulated STT transcript/complete/blocked states.
- Mic cutoff blocks voice commands without chat routing, TTS output, proposal creation, or execution.
- Voice command replies default to `options.voice.speak_response=true` while preserving explicit
...
Phase 0AU implemented:

- Added `guidedActionPanel` below the chat status line.
- Added `renderGuidedAction()`, `applyGuidedAction()`, `selectGuidedProposal()`, and
  `selectGuidedPlan()` dashboard helpers.
- Chat responses with `result.next_action` now display the recommended action and select the target
  proposal or plan when present.
- Proposal selection clears proposal filters and refreshes if the guided target is hidden.
- Added dashboard hook tests proving the guided-action display and selection helpers remain present.

Previous Phase 0AT status: Metis chat can answer "what should I do next?" with the exact next governed UI/API
instruction for pending plan reviews, pending proposal reviews, approved proposals awaiting receipt
requests, bindable plan results, and empty/no-action states. These responses are instructions only:
chat still does not approve, deny, advance, request execution, or create receipts.

Phase 0AT implemented:

- Added chat recognition for `what should I do next`, `next governed action`, `next approval step`,
  `how do I approve`, and `how do I request execution`.
- Added `metis_tool_next_action.v0.1` response metadata with `recommended_action`, `target`,
  `ui_instruction`, `api_instruction`, `execution_allowed=false`, and
  `chat_may_perform_action=false`.
- Added explicit `proposal_id=...`, `proposal_...`, `plan_id=...`, and `plan_...` targeting for
  guided instruction prompts, including punctuation-tolerant parsing.
- Added tests for pending proposal review, approved proposal receipt request guidance, pending plan
  review, plan step proposal review, clear-workspace no-op guidance, and no mutation/execution.

Previous Phase 0AS status: Metis chat can summarize pending proposal approvals and execution receipts without
mutating review/execution state. The summaries are bounded to IDs, tool/risk/status labels, argument
keys, receipt status, policy decisions, output hashes, and output key names; raw file contents,
command output, secrets, and external receipts stay out of chat metadata.

Phase 0AS implemented:

- Added chat recognition for `what needs approval`, `pending approvals`, `proposal status`, and
  related approval-queue prompts.
- Added chat recognition for `receipt summary`, `execution receipts`, `audit receipts`, and related
  receipt-summary prompts.
- Added bounded `metis_tool_approval_status.v0.1` and `metis_tool_receipt_status.v0.1` chat
  response metadata.
- Proposal summaries include pending counts, review-status counts, proposal IDs, tool IDs,
  risk/side-effect labels, dry-run availability, execution_allowed=false, and argument key names only.
- Receipt summaries include receipt IDs, proposal IDs, tool IDs, execution status, policy decision,
  review status, output hash/key names, redaction labels, and execution_allowed=false.
- Added tests proving chat summaries are observational: they do not approve, deny, request execution,
  create receipts, expose dry-run result bodies, or mark external action executed.

Previous Phase 0AR status: Metis chat can answer plan-status questions and explicitly advance the latest or named
governed tool plan through the same gated `/advance` path. Chat still cannot approve plans, approve
proposals, grant standing authority, or bypass execution gates.

Phase 0AR implemented:

- Added chat recognition for `tool plan status`, `what's next for my tool plan`, and related status
  prompts.
- Added chat recognition for explicit `advance tool plan` / `continue tool plan` prompts, using the
  existing guided advance logic.
- Added optional `plan_id=...` / `plan_...` targeting in chat plan-control prompts.
- Added `tool_planner` responses with `metis_tool_plan_status.v0.1` or
  `metis_tool_plan_advance.v0.1` metadata.
- Added tests proving chat status reports the next gate, chat advance stops at review gates, approved
  plans can only queue step proposals, unknown plan IDs return 404, and no external action is marked
  executed.

Previous Phase 0AQ status: Metis has a PowerShell launch script and an explicit `METIS_REPO_ROOT` allowlist anchor
for approved read-only local lanes. The execution policy now distinguishes arbitrary/autonomous
execution, which remains disabled, from the scoped approved read-only receipt lanes that already
exist behind proposal review gates.

Phase 0AQ implemented:

- Added `scripts/launch_metis.ps1` for Windows/PowerShell launch.
- The launch script sets `METIS_REPO_ROOT`, switches to the repo root, and starts Uvicorn.
- `filesystem.read` and `git.status` resolve relative paths/repos against `METIS_REPO_ROOT` when set.
- Clarified `/metis/execution/policy` with `execution_enabled_meaning`,
  `scoped_read_only_receipts_enabled`, and `active_approved_read_only_lanes`.
- Updated governed-tool readiness wording from generic "execution disabled" to arbitrary execution
  disabled.
- Added tests for launch script content, configured repo-root behavior from a different cwd, and
  clarified execution policy fields.

Previous Phase 0V/AUDIO10 status: Piper voice visualization now generates spectrum frames based on actual WAV duration instead
...
The launch script sets `METIS_REPO_ROOT` to the checkout root before starting the app. That keeps
approved read-only `filesystem.read` and `git.status` lanes anchored to this repo even if the server

...
simulation-first scope. Practical tool-using task requests are about `60%` complete: Metis can plan
multi-step governed tool tasks, persist/review them, queue eligible step proposals, and execute narrow reviewed read-only lanes, but cannot yet run
multi-step plans, live fetch URLs, use BOH as a tool, mutate files, call Atlas, run shell commands,
or act autonomously.

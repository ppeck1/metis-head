...
| `renderRadioWave` | 0V/AUDIO10 | Renders one vertical mirrored spectrum frame, resamples real bands to fill the panel, uses utterance-level gain when present, and falls back to aggregate levels for compatibility. |
...
| `pulseRadioFromVoice` | 0V/AUDIO10 | Pulses the radio strip from returned voice event metadata, using `audio_spectrum_frames`, `audio_spectrum_levels`, `audio_levels`, `audio_visualization_hint_ms`, and utterance-level visual gain when available. |

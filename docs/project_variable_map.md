...
Current Phase 0S/0R/0T/0U/0W/0Q/0L/0G/0F/0J/0K/0N/0D/0E/0I/0H/0AA/0AB/0AC/0AD/0AE/0AF/0AG/0AH/0AI/0AJ/0AK/0AL/0AM/0AN/0AO/0AP/0V-AUDIO10/0V-AUDIO11/0V-AUDIO12/0AQ/0AR/0AS/0AT/0AU/0AV/0AW/0AX/0AY UI estimate: `94%` functional for simulation review. Core state/API/scenario panels work, the virtual radio can emit canonical events, event logs can be exported/replayed, virtual chat can call a governed LLM router, route explicit tool requests through `tool_router`, create persisted governed plans through `tool_planner`, answer plan-status questions, explicitly advance plans through the existing governed advance path, summarize approval queues and execution receipts without mutating state, give exact next-step UI/API instructions without performing them, project those instructions into dashboard selection shortcuts, accept simulated voice commands through the same governed chat/tool route, answer tool/capability awareness questions through a deterministic registry-derived `tool_capability` route before provider generation, apply explicit simulated voice confirmation to one pending proposal without execution authority, and show a passive dashboard `Voice Trace` from redacted command/confirmation events, the dashboard can select locally available Ollama models, and the Tools panel can inspect the registry, dry-run safe tools, queue proposals, review proposals, request execution receipts, inspect the audit log, review the read-only execution policy, view the tool contract manifest, view the composed policy snapshot, evaluate tool gates, view governed-tool readiness, view governed-tool completion, plan governed tool tasks, persist/review plan records, queue approved plan step proposals, request execution for individually approved plan step proposals, bind safe receipt summaries into dependent plan steps, advance plans through governed non-review transitions, and exercise approved `time.now`, `git.status`, and `filesystem.read` read-only lanes. As of Phase 0J, chat `git status` and `read/open file` intents queue the active approved read-only proposal lanes, not the legacy placeholder lanes. As of Phase 0K, chat `fetch ...` queues blocked fetch proposals and `plan:` returns visible planning dry-runs. As of Phase 0N, replay and receipt-detail tests cover blocked fetch proposals and dry-run-only planning receipts. As of Phase 0D, tool catalog/detail responses expose lifecycle labels. As of Phase 0E, `search boh`, `retrieve boh`, and `search library` chat intents queue `boh.retrieve_proposed` without live BOH retrieval. As of Phase 0I, proposal listings can be filtered by status, proposal type, and tool ID. As of Phase 0H, tool catalog/detail responses expose `permission_requirements` metadata. As of Phase 0AA, `/metis/tools/contract` exports a governed contract manifest with registry counts, lanes, matrix rows, and boundary text. As of Phase 0AB, `/metis/tools/policy_snapshot` composes the contract, read-only policy, proposals, receipts, and authority flags into one review packet. As of Phase 0AC, tool arguments are validated against manifest input schemas before dry-run, proposal, execute, or chat tool routing proceeds. As of Phase 0AD, `/metis/tools/governance/evaluate` provides advisory gate decisions without mutating state. As of Phase 0AE, proposal review receipts and reviewed proposal records include single-proposal, non-standing review scope. As of Phase 0AF, `/metis/tools/readiness` computes a domain-labeled governed-tool readiness score. As of Phase 0AG, `/metis/tools/completion` computes `100%` completion for the current simulation-first governed tool substrate while listing future live lanes as out of scope. As of Phase 0AH, `/metis/tools/task/plan` turns broad task requests into deterministic reviewable tool plans. As of Phase 0AI, plans persist in canonical `tool_plan_queue` and can be listed/detail-read. As of Phase 0AJ, persisted plans can be approved or denied with non-standing review receipts, still without step execution. As of Phase 0AK, approved plans can queue eligible step proposals through the existing proposal queue without executing them. As of Phase 0AL, approved plan step proposals can have execution requested through existing receipt gates. As of Phase 0AM, bounded receipt summaries can be bound into later pending dry-run steps without raw-content leakage. As of Phase 0AN, `/advance` computes the next governed action and stops at human review gates. As of Phase 0AO, explicit chat planning prefixes queue persisted governed plans and return the first `next_action` without approval, proposal materialization, result binding, or execution. As of Phase 0AP, Ollama/OpenAI/mock prompts state that governed tools exist while the LLM provider does not call them directly. As of Phase 0V/AUDIO10, Piper `audio_spectrum_frames` scale with actual WAV duration and preserve frame loudness through utterance-level dashboard gain. As of Phase 0V/AUDIO11, the radio meter is a fixed 32-row by 8-segments-per-side mirrored LED-cell preview, Piper exposes exact WAV `audio_duration_ms`, and dashboard spectrum playback advances once without looping. As of Phase 0V/AUDIO12, the preview uses hardware-oriented inactive cells, restrained glow, outward green/yellow/orange tiers, adjacent near-limit peak markers, and dashboard-only speech conditioning for a fuller analog analyzer profile without changing backend spectrum metadata. As of Phase 0AQ, `scripts/launch_metis.ps1` sets `METIS_REPO_ROOT`, and the read-only local lanes use that root when configured. As of Phase 0AR, chat plan-status and explicit plan-advance prompts report or invoke only the existing gated plan transitions and cannot approve plans/proposals or grant execution. As of Phase 0AS, chat approval/receipt summaries expose bounded safe metadata and cannot approve, deny, request execution, or create receipts. As of Phase 0AT, chat next-action guidance names the exact UI/API action required while preserving explicit operator action. As of Phase 0AU, the dashboard displays next-action guidance and selects the relevant proposal or plan without clicking any governed action button. As of Phase 0AV, simulated voice commands are tool-aware and routed through canonical chat/tool governance while real mic/STT remains future work. As of Phase 0AW, direct questions about tools/capabilities return deterministic `metis_tool_capability_awareness.v0.1` metadata and wording through typed or spoken routes, avoiding provider drift such as "I have no tools." As of Phase 0AX, `/metis/voice/confirm` requires explicit proposal-specific approve/deny phrasing, returns readback for ambiguity, supports cancellation without review, and never requests execution. As of Phase 0AY, the dashboard renders redacted simulated voice event traces without raw transcript/audio and without adding action buttons. Governed tools substrate: `100%` complete for the current simulation-first scope. Practical tool-using task requests: about `96%` complete; general live data-dependent plan execution, live fetch, BOH-as-tool, Atlas/tool adapters, writes, shell, hardware actions, external mutation, real STT/wake-word, and autonomy remain future-phase work.
...
Virtual Radio is a 3-zone instrument (`grid-template-columns: 58% 19% 23%`): an inert `radio-speaker` grille (visual only), a tuning-window `radio-strip` carrying the activity LED, authority LED, and full-panel vertical mirrored `radio-meter` spectrum analyzer, and a right `radio-controls` stack (Volume + Depth knobs, PWR/LOUD/AFC/AM-FM `radio-switches`, and the large Tuning/Initiative knob). The analyzer is a hardware-parity simulator rendering of the buildspec tuning-window LED/status visualizer: `32` vertical frequency rows, `8` amplitude segments per mirrored side, a subdued center seam, dark inactive cells, discrete outward green/yellow/orange LED cells, and red peak markers limited to near-saturation rows. It is not final LED firmware or a selected physical panel. Power/audio/mode/authority readouts and the mic/camera cutoff buttons were moved out of the radio face into the `Radio Status` panel. Virtual Chat's Send button is attached to the composer textarea (Enter sends, Shift+Enter inserts a newline, Send disables while generating); Clear Input is a secondary action. Control meanings are unchanged.
...
| `provider_event` (`tts`) | 0V/AUDIO9 | `status`, `voice_provider`, `voice_id`, `voice_schema`, `text_len`, `text_hash`, `text_redacted`, `normalized_text`, `source_text_len`, `source_text_hash`, `playback_strategy`, `playback_mode`, `audio_visualization_hint_ms`, `audio_levels`, `audio_level_count`, `audio_spectrum_levels`, `audio_spectrum_count`, `audio_spectrum_frames`, `audio_spectrum_frame_count`, `audio_spectrum_rows`, `audio_spectrum_segments_per_side`, optional `audio_duration_ms`, optional `audio_file=local_temp_wav` | Voice output events; raw spoken text, raw audio, and concrete temp paths are not persisted. `audio_spectrum_frames` and `audio_spectrum_levels` are derived from the actual Piper WAV and drive the mirrored analyzer. |
| `provider_event` (`stt`) | 0AV | `status=transcript|complete|blocked`, `input_mode=simulated_voice_command`, `text_len`, `text_hash`, `text_redacted=true`, optional `reason` | Simulated voice-command recognition events. Raw audio and raw transcript are not stored in provider events. |
...
| `_route_chat_plan_control_request(message)` | 0AR | Detects explicit chat plan-status and plan-advance prompts without invoking an LLM. |
| `_chat_plan_status(plan_id)` | 0AR | Returns the latest or specified governed plan with its next action; does not mutate state. |
| `_chat_plan_advance(plan_id)` | 0AR | Invokes the existing `/advance` logic for the latest or specified governed plan; cannot approve plans or proposals. |
| `_route_chat_queue_status_request(message)` | 0AS | Detects approval-queue and receipt-summary prompts without invoking an LLM. |
| `_proposal_status_summary(limit)` | 0AS | Builds a bounded approval-queue report with IDs, labels, counts, and argument keys only. |
| `_receipt_status_summary(limit)` | 0AS | Builds a bounded execution-receipt report with IDs, status labels, output hashes/key names, and redaction labels only. |
| `_route_chat_next_action_request(message)` | 0AT | Detects explicit next-step/operator-instruction prompts without invoking an LLM. |
| `_next_action_instruction(plan_id, proposal_id)` | 0AT | Chooses the next governed plan/proposal/receipt action and returns UI/API instructions without mutating state. |
| `_proposal_instruction(proposal)` | 0AT | Builds proposal review, execution-request, receipt-inspection, or no-op instruction payloads. |
| `_plan_instruction(plan)` | 0AT | Builds plan review, step queue, proposal review, execution-request, bind-result, or no-op instruction payloads from `next_plan_action`. |
...
| `OllamaLLMProvider` | 0R/0AP | Calls local Ollama `/api/chat`; no direct tools or retrieval, but receives truthful governed-tool capability context in messages. |
| `OpenAILLMProvider` | 0R/0AP | Calls OpenAI Chat Completions; no direct tools or retrieval, but receives truthful governed-tool capability context in messages. |
...
| `_governed_tool_capability_context(state)` | 0AP | Builds registry-derived system prompt text so LLM providers can describe governed tool lanes truthfully without claiming direct tool calls. |
...
| `_wav_spectrum_envelope` | 0V/AUDIO7/AUDIO11 | Extracts fixed-row frequency-band levels from the generated Piper WAV for the vertical mirrored analyzer. |
| `_wav_spectrum_frames` | 0V/AUDIO10 | Extracts duration-scaled, loudness-preserving time-sliced frequency-band frames from the generated Piper WAV for animated analyzer motion. |
| `_spectrum_frame_count` | 0V/AUDIO10 | Computes default analyzer frame count from WAV duration, targeting smooth motion while capping event-log size. |
| `_rms_level` | 0V/AUDIO10 | Computes per-segment energy used to preserve quiet/loud frame differences. |
...
| `voice_command(payload)` | 0AV | Simulated recognized voice-command route; emits redacted STT events, routes through chat, and defaults to spoken response. |
...
| `tool_capabilities` | 0AW | `/metis/chat` and `/metis/voice/command` response metadata for capability-awareness prompts; includes safe dry-run tools, approved read-only lanes, proposal-only lanes, `llm_direct_tool_calling=false`, `voice_instruction_supported=true`, and boundary text. |
| `voice_confirmation` | 0AX | `/metis/voice/confirm` response metadata; includes recognized status, decision, proposal ID, confirmation acceptance, `execution_allowed=false`, and `standing_approval=false` when a review is applied. |
| `readback` | 0AX | Safe `metis_voice_confirmation_readback.v0.1` proposal summary returned before or alongside simulated voice confirmation; excludes raw tool output and secrets. |
...
| `execution_enabled` | 0Q/0AQ | Always `false` for arbitrary/autonomous execution; scoped approved read-only receipt lanes are represented separately. |
| `execution_enabled_meaning` | 0AQ | Operator-facing explanation distinguishing arbitrary execution from gated read-only receipts. |
| `scoped_read_only_receipts_enabled` | 0AQ | Boolean showing that reviewed `time.now`, `filesystem.read`, and `git.status` receipt lanes are active. |
| `active_approved_read_only_lanes` | 0AQ | Structured list: `time.now`, `filesystem.read`, `git.status`. |
| `scripts/launch_metis.ps1` | 0AQ | PowerShell launch script; sets `METIS_REPO_ROOT`, changes to repo root, and starts Uvicorn. |
...
| `radioMeter` | 0V/AUDIO9/AUDIO11/AUDIO12 | Virtual tuning-window visualizer; renders an idle center seam and a full-height bottom-to-top mirrored `32 x (8 + 8)` discrete LED-cell analyzer from Piper `audio_spectrum_frames` during speech. Dark inactive cells remain visible, active tiers progress outward from green to yellow to orange, and red peak markers appear only near saturation. |
...
| `chatStatus` | 0R/0AO/0AS | Provider/proposal/source/failure status line; shows `plan_queued`, `tool_planner`, approval summaries, and receipt summaries for governed chat-control responses. |
| `guidedActionPanel` | 0AU | Displays `metis_tool_next_action.v0.1` recommended action, target, UI instruction, and API route hints from chat. |
...
| `voiceTrace` | 0AY | Passive dashboard trace of redacted simulated voice-command and voice-confirmation provider events. |
...
| `safeVoiceTraceEvents` | 0AY | Filters canonical event log down to redacted simulated voice command/confirmation events for dashboard display. |
| `renderVoiceTrace` | 0AY | Renders command/confirmation status, proposal ID, text length/hash, redaction status, and safe reason labels in `voiceTrace`. |
| `pulseRadioAudio` | 0V/AUDIO11 | Pulses the virtual radio meter/strip when TTS output is active or newly completed; advances returned spectrum frames once without looping and decays after the final frame. |
| `renderRadioWave` | 0V/AUDIO11/AUDIO12 | Renders one fixed-geometry vertical mirrored LED-cell spectrum frame, resamples real bands to 32 rows, applies dashboard-only conditioning plus utterance-level gain, quantizes each side to 8 amplitude segments, and falls back to aggregate levels for compatibility. |
...
| `pulseRadioFromVoice` | 0V/AUDIO11 | Pulses the radio strip from returned voice event metadata, preferring exact `audio_duration_ms` and using `audio_spectrum_frames`, aggregate compatibility levels, visualization hints, and utterance-level visual gain when available. |
| `visualGainForVoice` | 0V/AUDIO10 | Computes one analyzer gain across the whole utterance so backend frame loudness is not flattened per frame. |
| `conditionRadioLevels` | 0V/AUDIO12 | Applies light neighboring-row spread before display quantization so ordinary speech reads as a fuller analog analyzer profile without mutating Piper metadata. |
| `resetRadioPeaks` | 0V/AUDIO11 | Resets mirrored LED peak-hold state for a new utterance or idle transition. |
| `updateRadioPeak` | 0V/AUDIO11/AUDIO12 | Applies restrained per-row peak hold and decay, clamps markers directly beside illuminated bars, and suppresses red markers below the near-limit threshold. |
| `createSpectrumCell` | 0V/AUDIO11 | Creates one discrete inactive/active/peak LED cell for the virtual matrix preview. |
...
| `renderGuidedAction` | 0AU | Renders chat next-action guidance into `guidedActionPanel`; display only. |
| `applyGuidedAction` | 0AU | Applies a chat guidance target to dashboard selection controls; never clicks approval/execution buttons. |
| `selectGuidedProposal` | 0AU | Selects a proposal in `proposalSelect`, clearing filters and refreshing if the target is hidden. |
| `selectGuidedPlan` | 0AU | Selects a tool plan in `toolPlanSelect` after refreshing plans if needed. |
...
| `POST` | `/metis/chat` | `metis_head.brain` | Governed virtual chat through selected LLM provider, `tool_router` for explicit governed tool requests, `tool_planner` for explicit governed planning/control prompts, or `tool_capability` for deterministic registry-derived capability-awareness answers. When source grounding is on and BOH enabled (0B), retrieves read-only context first; response adds `source_state`, `metadata.boh`, and `retrieval`. Tool routing can be disabled per request with `options.tools.enabled=false`; Phase 0J chat routing queues active `git.status` and `filesystem.read` proposals without direct execution; Phase 0K routes `fetch ...` to blocked proposals and `plan:` to visible dry-runs; Phase 0AO routes `plan task:` and related prefixes to persisted governed plans and returns the first `next_action`; Phase 0AR routes `tool plan status`/`what's next for my tool plan` and explicit `advance tool plan`/`continue tool plan` prompts to the existing governed plan status/advance paths; Phase 0AS routes approval-queue and receipt-summary prompts to bounded observational reports; Phase 0AT routes next-step prompts to exact UI/API instructions without performing them; Phase 0AW answers direct tool/capability questions before LLM provider generation and returns `tool_capabilities`. |
...
| `POST` | `/metis/voice/command` | `metis_head.brain` | Simulated voice-command ingress for future radio operation. Accepts recognized text/transcript, emits redacted STT events, routes to `/metis/chat`, requests spoken response by default, blocks on mic cutoff, and uses the Phase 0AW deterministic `tool_capability` route for spoken tool-awareness questions. |
| `POST` | `/metis/voice/confirm` | `metis_head.brain` | Simulated voice confirmation for one pending proposal. Requires explicit approve/deny phrasing plus proposal ID, returns `readback_required` for ambiguity, supports cancellation without review, reuses existing proposal review events, and never requests execution. |
...
| `GET` | `/metis/execution/policy` | `metis_head.brain` | Return `metis_read_only_execution_policy.v0.1`; arbitrary execution remains disabled while scoped approved read-only receipt lanes are listed separately. |

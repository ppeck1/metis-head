...
| `POST` | `/metis/voice/command` | `metis_head.brain` | Simulated voice-command ingress for future radio operation. Accepts recognized text/transcript, emits redacted STT events, routes to `/metis/chat`, requests spoken response by default, and blocks on mic cutoff. |

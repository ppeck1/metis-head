...
| Last capsule audit | 2026-07-01T19:30:00-04:00 |
...
| METIS_LLM_PROVIDER | Chat provider selector | mock | no | process | yes |
| METIS_OLLAMA_BASE_URL | Local Ollama base URL; loopback HTTP only for the tool adapter | `http://127.0.0.1:11434` | no | host | yes |
| METIS_OLLAMA_MODEL | Ollama model used by ordinary chat/tool orchestration | unset | no | process | yes |
...
| METIS_STT_ALLOW_LOCAL | Enables local faster-whisper execution | false | no | process | yes |
| METIS_STT_ENGINE | Selected STT adapter | simulated | no | process | yes |
| METIS_STT_MODEL | Warmed faster-whisper model name | small | no | process | yes |
| METIS_STT_MODEL_DIR | Optional local model/download directory | unset | no | host | yes |
| METIS_AUDIO_ALLOW_LOCAL_MIC | Enables server-host microphone adapter | false | no | process | yes |
